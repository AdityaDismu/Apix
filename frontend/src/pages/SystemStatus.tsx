...
function StatusBadge({
  ...
  const className = healthy
    ? 'border-[#C5DDCC] bg-[#EEF5F0] text-[#4E8066]'
    : warning
      ? 'border-[#E1CFAB] bg-[#F7F2E7] text-[#95672D]'
      : 'border-[#E7C1BC] bg-[#F7ECEA] text-[#A85C57]';
    ...
    <div className="page-shell">
      ...
      {error ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card className="border-2 border-[#6B5A78] bg-[#F0EDE7]">
            <div className="section-label text-[#6B5A78]">NATIONAL APIx</div>
            <h2 className="mt-1 text-2xl font-semibold tracking-tight text-[#30313A]">
              Current publication state
            </h2>
            <p className="mt-2 text-sm text-[#74727A]">
            ...
            {latestNational ? (
              <div className="mt-8">
                <div className="text-center md:text-left">
                  <div className="text-5xl font-bold tracking-tight text-[#30313A]">
                    {latestNational.index.toFixed(4)}
                  </div>
                  <div className="mt-1 text-sm font-semibold uppercase tracking-wider text-[#74727A]">
                    National APIx
                  </div>
                </div>

                <div className="my-8 border-y border-[#DCD7CE] py-6">
                  <div className="text-lg font-semibold text-[#30313A]">
                    {(latestNational.route_coverage_ratio * 100).toFixed(0)}% route coverage
                  </div>
                  <div className="mt-1 text-sm text-[#74727A]">
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
                  <div>
                    <div className="text-xs text-[#74727A]">Observation date</div>
                    <div className="mt-1 font-medium text-[#30313A]">{latestNational.observation_date}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#74727A]">Weight reference</div>
                    <div className="mt-1 font-medium text-[#30313A]">{latestNational.weight_reference_period || 'DGCA 2024–25'}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#74727A]">Publication gate</div>
                    <div className="mt-1 font-semibold text-[#4E8066]">
                      {latestNational.route_coverage_ratio >= 0.8 ? 'PASSED' : 'FAILED'}
                    </div>
                  </div>
                </div>

                <div className="mt-8 rounded-xl border border-[#DCD7CE] bg-white p-5">
                  <h3 className="text-sm font-bold text-[#30313A]">What does this mean?</h3>
                  <p className="mt-2 text-sm leading-relaxed text-[#74727A]">
            ...
            <Card>
              ...
              <ul className="mt-6 space-y-3 text-sm">
                <li className="flex items-center gap-3">
                  <span className="w-28 text-[#74727A]">Backend</span>
                  <span className={`font-medium ${healthStatus === 'ok' ? 'text-[#4E8066]' : 'text-[#95672D]'}`}>
                    ● {healthStatus === 'ok' ? 'Operational' : healthStatus}
                  </span>
                </li>
                <li className="flex items-center gap-3">
                  <span className="w-28 text-[#74727A]">Database</span>
                  <span className={`font-medium ${databaseStatus === 'ok' ? 'text-[#4E8066]' : 'text-[#95672D]'}`}>
                    ● {databaseStatus === 'ok' ? 'Connected' : databaseStatus}
                  </span>
                </li>
                <li className="flex items-center gap-3">
                  <span className="w-28 text-[#74727A]">Source adapter</span>
                  <span className="font-medium text-[#30313A]">
                    ● {sources[0]?.name || 'Google Flights'}
                  </span>
                </li>
                <li className="flex items-center gap-3">
                  <span className="w-28 text-[#74727A]">Route basket</span>
                  <span className="font-medium text-[#30313A]">
              ...
              <div className="mt-6 rounded-lg bg-[#F6F2EC] p-4 text-sm text-[#74727A]">
            ...
            <Card>
              ...
              {latestPipeline ? (
                <div>
                  ...
                  <ul className="mt-6 space-y-2 text-base font-medium text-[#30313A]">
                    ...
                    {latestPipeline.routes_failed > 0 && (
                      <li className="text-[#A85C57]">{latestPipeline.routes_failed} failed routes</li>
                    )}
                  </ul>
                  
                  <div className="mt-4 text-xs font-medium text-[#74727A] flex items-center gap-2">
                    <Timer size={14} />
                    {formatDate(latestPipeline.started_at)}
                  </div>

                  <p className="mt-6 text-sm text-[#74727A] pt-4 border-t border-[#DCD7CE]">
          ...
          <Card>
             ...
             <div className="flex items-center gap-3">
                <h3 className="text-lg font-semibold text-[#30313A]">
                  {sources[0]?.name || 'Google Flights'} adapter — {sources[0]?.enabled === false ? 'Disabled' : 'Active'}
                </h3>
             </div>

             <p className="mt-3 text-sm leading-relaxed text-[#74727A] max-w-3xl">
             ...
             <div className="mt-5 flex gap-8 text-sm border-t border-[#DCD7CE] pt-5">
               <div><span className="text-[#74727A]">Route reference:</span> <span className="font-medium text-[#30313A]">DGCA 2024–25</span></div>
               <div><span className="text-[#74727A]">Methodology:</span> <span className="font-medium text-[#30313A]">v4.0</span></div>
             </div>
          </Card>

          {/* System Principles Prominently Displayed */}
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <Card className="bg-[#655B68] text-white border-0 shadow-lg">
              <div className="flex items-center gap-2">
                <ShieldCheck size={18} className="text-[#74A385]" />
                <div className="text-[10px] font-bold uppercase tracking-widest text-[#9A9499]">
                  NO FABRICATION
                </div>
              </div>
              <h3 className="mt-4 text-xl font-semibold">Integrity by design</h3>
              <p className="mt-3 text-sm leading-relaxed text-[#CDC5BB]">
            ...
            <Card className="border-[#DCD7CE] shadow-sm">
              <div className="flex items-center gap-2">
                ...
                <div className="text-[10px] font-bold uppercase tracking-widest text-[#74727A]">
                  TRACEABILITY
                </div>
              </div>
              <h3 className="mt-4 text-xl font-semibold text-[#30313A]">Auditable data pipeline</h3>
              <p className="mt-3 text-sm leading-relaxed text-[#74727A]">
          ...
          <details className="group mt-4 mb-8">
            <summary className="flex cursor-pointer items-center gap-1 text-sm font-medium text-[#6B5A78] hover:underline">
              <ChevronRight size={16} className="transition-transform group-open:rotate-90" />
              View technical diagnostics
            </summary>
            <div className="mt-4 rounded-xl border border-[#DCD7CE] bg-[#F6F2EC] p-6 text-sm text-[#74727A]">
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-wide">Stored observations</div>
                  <div className="mt-1 text-lg font-semibold text-[#30313A]">{formatNumber(quality?.observations_total)}</div>
                </div>
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-wide">Valid observations</div>
                  <div className="mt-1 text-lg font-semibold text-[#30313A]">{formatNumber(quality?.observations_valid)}</div>
                </div>
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-wide">Flagged</div>
                  <div className="mt-1 text-lg font-semibold text-[#30313A]">{formatNumber(quality?.observations_flagged)}</div>
                </div>
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-wide">Collection runs</div>
                  <div className="mt-1 text-lg font-semibold text-[#30313A]">{formatNumber(quality?.collection_runs)}</div>
                </div>
              </div>
            </div>
          </details>
          ...
          <div className="pb-10 pt-4 flex items-center justify-center gap-2 text-xs text-[#74727A]">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#5D8D70] opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-[#5D8D70]"></span>
            </span>
            <p>
              <b className="text-[#4B4851]">Live system status</b> · Values shown on this page are retrieved from the current backend state, not static demonstration data.
            </p>
          </div>
